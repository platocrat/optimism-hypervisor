...
describe("Hypervisor", () => {
  ...
  beforeEach("deploy contracts", async () => {
    ({ token0, token1, token2, factory, router, nft, hypervisorFactory } = await loadFixture(
      hypervisorTestFixture
    ));
  ...
  it("multiple deposits and total withdrawal", async () => {
    ...
    expect(
      token0BeforeRebalanceSwap.sub(token0AfterRebalanceSwap).sub(rebalanceSwapAmount).abs()
    ).to.be.lt(ethers.utils.parseEther("1"));
    token0hypervisor = await token0.balanceOf(hypervisor.address);
    token1hypervisor = await token1.balanceOf(hypervisor.address);
    expect(token0hypervisor).to.equal(0);
    expect(token1hypervisor).to.equal(0);
    ...
    expect(token0AfterSecondRebalance.sub(token0BeforeRebalanceSwap).abs()).to.be.lt(
      ethers.utils.parseEther("15")
    );
    expect(token1AfterSecondRebalance.sub(token1BeforeRebalanceSwap).abs()).to.be.lt(
      ethers.utils.parseEther("15")
    );
...
describe("ETHUSDT Hypervisor", () => {
  const [wallet, other, user0, user1, user2, user3, user4, user5] =
    waffle.provider.getWallets();

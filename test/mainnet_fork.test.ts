...
describe.skip("Hypervisors on Mainnet Fork", () => {
  ...
  beforeEach("deploy contracts", async () => {
    ...
    hypervisorFactory = (await hypervisorFactoryFactory.deploy(
      uniswapV3Factory
    ...
    const usdcEthHypervisorAddress = await hypervisorFactory.getHypervisor(
      usdcAddress,
      wethAddress,
      FeeAmount.MEDIUM
    );
    hypervisor = (await ethers.getContractAt("Hypervisor", hypervisorAddress)) as Hypervisor;
    usdcEthHypervisor = (await ethers.getContractAt(
      "Hypervisor",
      usdcEthHypervisorAddress
    )) as Hypervisor;
